...
const wallets = [
    ...
    },
    {
        ...
        "proposals": [
            ...
            {
                "proposalId": "0x188b6515af07e339f55f1581001sda0db298781aab64261d484dd4cc09246ce85",
                "timestamp": 1735872933722,
                "outAssetId": 1027,
                "outTokens": 1500.50,
                "blockchain": "Gnosis chain",
                "classification": "New investment",
                "initiator": 3,
                "toAddress": "0x98ec059dc3adfbdd63429454aeb0c990fba4a128",
                "approvals": [
                    { "approverId": 1 },
        ...
        "proposals": [
            ...
            },
            {
                ...
                "toAddress": [
                    { "toAddress": "0x191CB52B345E422D318BB019D9ce584D95a3788c" },
                    { "toAddress": "0xc9a3a4B124934fcc2D8EDDD14b4A031cd925922c" },
                    { "toAddress": "0x3258841f1a169B7639BE2ff44342A05E38A883f" }
                ],
                "approvals": [
                    { "approverId": 1 },
                ],
...
// Function to update dates dynamically
function updateWalletDates(wallets) {
    const today = new Date().getTime();
    const twelveMonthsAgo = today - (365 * 24 * 60 * 60 * 1000);
    ...
    wallets.forEach(wallet => {
        ...
        wallet.proposals.forEach(proposal => {
            allTimestamps.push(proposal.timestamp);
        });
    });
    
    // Find min and max
    const originalMin = Math.min(...allTimestamps);
    const originalMax = Math.max(...allTimestamps);
    const originalRange = originalMax - originalMin;
    const targetRange = today - twelveMonthsAgo;
    
    // Map timestamps to new range
    const mapTimestamp = (oldTimestamp) => {
        const positionInOriginal = (oldTimestamp - originalMin) / originalRange;
        return twelveMonthsAgo + Math.floor(positionInOriginal * targetRange);
    };
    
    // Update all timestamps
    wallets.forEach(wallet => {
        wallet.transactions.forEach(tx => {
            tx.timestamp = mapTimestamp(tx.timestamp);
        });
        
        wallet.proposals.forEach(proposal => {
            proposal.timestamp = mapTimestamp(proposal.timestamp);
        });
    });
...
// Function to get updated wallets with current dates
function getUpdatedWallets() {
    // Check if we need to update (in a browser environment)
    if (typeof window !== 'undefined' && window.localStorage) {
        const today = new Date();
        const lastUpdateDate = localStorage.getItem('walletsLastUpdate');
        
        if (!lastUpdateDate || new Date(lastUpdateDate).toDateString() !== today.toDateString()) {
            const updatedWallets = updateWalletDates(wallets);
            localStorage.setItem('walletsLastUpdate', today.toISOString());
